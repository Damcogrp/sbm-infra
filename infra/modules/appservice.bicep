...
//   Frontend: app-sbm-{env}-{region}-fe  (Angular v21, Node 22)
//   Backend:  app-sbm-{env}-{region}-be  (.NET 10, Linux)
//   EventBus: func-sbm-{env}-{region}-evb (.NET 10 isolated, Linux)
// ============================================================
...
resource frontendApp 'Microsoft.Web/sites@2022-09-01' = {
  ...
  properties: {
    ...
    siteConfig: {
      ...
      linuxFxVersion: 'NODE|22-lts'    // ← Node 22 for Angular frontend
      appSettings: [
...
// ── Backend App (.NET 10, Linux + Nginx) ───────────────────────
resource backendApp 'Microsoft.Web/sites@2022-09-01' = {
  name: beAppName
  location: location
  kind: 'app,linux'            // ← Linux app
  identity: { type: 'SystemAssigned' }
  tags: union(tags, { Component: 'backend', Stack: 'dotnet-10', OS: 'linux', WebServer: 'nginx' })
  properties: {
    serverFarmId: appServicePlan.id
    httpsOnly: true
    siteConfig: {
      ...
      linuxFxVersion: 'DOTNETCORE|10.0'  // ← .NET 10 on Linux
      appSettings: [
...
// ── EventBus Function App (Linux, .NET 10 isolated) ───────────
resource functionApp 'Microsoft.Web/sites@2022-09-01' = {
  ...
  properties: {
    ...
    siteConfig: {
      minTlsVersion: '1.2'
      alwaysOn: true
      linuxFxVersion: 'DOTNET-ISOLATED|10.0'  // ← .NET 10 isolated on Linux
      appSettings: [
